...
export function Header(){return <header className="site-header"><Link className="wordmark" href="/">ONDUU<span>.</span></Link><nav aria-label="Primary navigation"><Link href="/readiness">Readiness</Link><Link href="/how-it-works">How it works</Link><Link href="/paths">Paths</Link><Link href="/guides">Guides</Link><Link href="/about">About</Link></nav><Link className="button button-small" href="/readiness">Check Your Digital Readiness</Link></header>}

export function Footer(){return <footer><div className="footer-brand"><Link className="wordmark" href="/">ONDUU<span>.</span></Link><p>Practical Digital Readiness guidance for better website and infrastructure decisions.</p><p className="footer-disclosure">Onduu is operated by Ujiajiri Enterprises Limited. Independent implementation partners contract with clients directly. HOSTAFRICA provides, bills and supports HOSTAFRICA products referenced on this site. Material referral, commission or sponsorship relationships are disclosed at the relevant decision point.</p></div><div><b>Start</b><Link href="/readiness">Readiness</Link><Link href="/how-it-works">How It Works</Link><Link href="/contact">Contact</Link></div><div><b>Choose a Path</b><Link href="/paths/website-and-digital-marketing">Ujiajiri Partners</Link><Link href="/paths/hostafrica-infrastructure">HOSTAFRICA Infrastructure</Link></div><div><b>Learn</b><Link href="/guides">Guides</Link><Link href="/insights">Insights</Link><Link href="/labs">Labs</Link><Link href="/guides/website-revenue-system">Website Revenue System</Link><Link href="/check">Email Security Check</Link><Link href="/domains">Domain Search</Link><Link href="/scan">Readiness Scan</Link></div><div><b>Legal</b><Link href="/legal/commercial-relationships">Commercial Relationships</Link><Link href="/legal/privacy">Privacy</Link><Link href="/legal/assessment-terms">Assessment Terms</Link></div></footer>}
...
export type ContentSection={eyebrow?:string;title:string;body?:string[];items?:string[];cards?:{title:string;body:string;meta?:string}[];steps?:{title:string;body:string}[];note?:string;links?:{label:string;href:string;external?:boolean}[]};
...
export function StandardPage({page,children}:{page:PageContent;children?:ReactNode}){return <><Header/><main><section className="page-hero"><div><p className="eyebrow">{page.eyebrow||"ONDUU / DIGITAL READINESS"}</p><h1>{page.title}</h1><p className="lede">{page.intro}</p>{page.cta&&<Button href={page.ctaHref||"/contact"}>{page.cta}</Button>}</div><aside className="hero-index"><span>Assess.</span><span>Prioritise.</span><span>Choose a path.</span><span>Verify.</span></aside></section>{page.gate&&<div className="gate"><b>PREVIEW / APPROVAL GATE</b><span>{page.gate}</span></div>}{page.sections.map((s,i)=><section className="content-section" key={s.title}><div><p className="section-number">{String(i+1).padStart(2,"0")} / {s.eyebrow||"DETAIL"}</p><h2>{s.title}</h2></div><div className="section-body">{s.body?.map(p=><p key={p}>{p}</p>)}{s.items&&<ul>{s.items.map(x=><li key={x}>{x}</li>)}</ul>}{s.cards&&<div className="content-cards">{s.cards.map(c=><article key={c.title}>{c.meta&&<small>{c.meta}</small>}<h3>{c.title}</h3><p>{c.body}</p></article>)}</div>}{s.steps&&<div className="steps">{s.steps.map((x,n)=><article key={x.title}><b>{String(n+1).padStart(2,"0")}</b><div><h3>{x.title}</h3><p>{x.body}</p></div></article>)}</div>}{s.links&&<div className="actions">{s.links.map(l=>l.external
  ...
  :<Link key={l.href} className="button" href={l.href}>{l.label}<span aria-hidden="true">↗</span></Link>)}</div>}{s.note&&<div className="note">{s.note}</div>}</div></section>)}{page.form&&<FormSection type={page.form}>{children}</FormSection>}<FinalCta href={page.form==="readiness"?"#request":"/readiness"}/></main><Footer/></>}
...
function FinalCta({href="/readiness"}:{href?:string}){return <section className="final-cta"><p className="section-number">YOUR NEXT STEP</p><h2>Start with the problem - not the supplier.</h2><p>Identify the priorities, understand the responsibility and then continue through the route that fits the work.</p><div className="actions"><Button href={href}>Check Your Digital Readiness</Button><Link className="text-link" href="/paths/website-and-digital-marketing">Find an Implementation Partner ↗</Link></div></section>}
