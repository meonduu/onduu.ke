    ...
    "sections": [
      ...
      },
      {
        ...
        "body": [
          ...
          "The domain search at /kedomains checks whether a name is registered in .co.ke and .ke using public DNS and registry records, and shows a taken domain's registrar, transfer lock and expiry. Nothing you search is stored. If an available name leads you to register it, that happens at HOSTAFRICA's own site — the outbound link carries attribution tags so Onduu can see, in aggregate, that traffic came from this tool, and Onduu counts the click as a number with no identity attached. Onduu receives no commission on registrations."
        ],
        "note": "The basis for storing scan results is a legitimate interest in operating the tool, caching results so it does not repeatedly fetch third-party sites, and being able to reproduce a score. A Public Signal Score describes public observations at a moment in time; it is not a Digital Readiness Score, and not proof that a domain, a mailbox or a business is secure. Stored scan results are kept until deleted rather than on a fixed schedule. If you operate a domain and do not want it scanned, email me@onduu.ke: any stored result for it will be deleted and the domain added to a do-not-scan list so it is not scanned again."
      },
      {
